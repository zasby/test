import { BasicScrollOptions, DefaultScrollOptions, LoopedFunction, Scroll } from "../types";
...
export class DefaultArtificialScroll implements AbstractArtificialScroll {
  ...
  protected getScrollMode = (scroll?: Scroll) => {
    ...
    if (overscroll && this.reference.current && scroll) {
      const { scrollLeft, scrollTop, clientHeight, clientWidth, scrollHeight, scrollWidth } = this.reference.current;
  ...
  protected calculateScroll(scroll: Scroll): PartitionedScroll {
    ...
    // TODO: Check hook props
    const rubberBand = false;
    ...
    if (this.mode === "global") {
      ...
      if (this.reference.current) {
        const { scrollLeft, scrollTop, scrollWidth, clientWidth, scrollHeight, clientHeight } = this.reference.current;
