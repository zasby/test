...
export const useTimer = (): IUseTimer => {
  ...
  const [currentIntervalNode, setCurrentIntervalNode] = useState<NodeJS.Timeout | undefined>(undefined);
  ...
  const timer = () => {
    ...
    if (timeMinutesRef.current <= 0) {
      ...
      const remainingTimeStr = `${strMinutes}:${strSeconds.toString().length < 2 ? "0" + strSeconds : strSeconds}`;
