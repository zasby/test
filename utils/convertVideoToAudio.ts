export const convertVideoToAudio = (videoFileData: File, targetAudioFormat: string = "mp3") => {
  try {
    ...
    return new Promise((resolve) => {
      reader.onload = function (event) {
        ...
        audioContext.decodeAudioData(videoFileAsBuffer as any).then(function (decodedAudioData) {
...
function writeString(s: string, a: any, offset: number) {
  for (var i = 0; i < s.length; ++i) {
    a[offset + i] = s.charCodeAt(i);
  }
}

function writeInt16(n: number, a: any, offset: number) {
  n = Math.floor(n);
...
function writeInt32(n: number, a: any, offset: number) {
  n = Math.floor(n);
  var b1 = n & 255;
  var b2 = (n >> 8) & 255;
  var b3 = (n >> 16) & 255;
...
function writeAudioBuffer(audioBuffer: AudioBuffer, a: any, offset: number) {
...
function uint8ToString(buf: any) {
  var i,
    length,
    out = "";
